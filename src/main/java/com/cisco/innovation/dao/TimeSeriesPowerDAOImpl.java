...
public class TimeSeriesPowerDAOImpl implements TimeSeriesPowerDAO{
	...
	public List getPowerUsageForUser(String username, String current, String end) {
		Query query = sessionFactory.getCurrentSession().createQuery("select timeSeriesPowerPK from TimeSeriesPowerData where timeSeriesPowerPK.dateTime <=:param1 AND timeSeriesPowerPK.dateTime >= :param2 AND username= :username");
